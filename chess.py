...
class Game:
    ...
    def menu_screen(self):
        ...
        menu = pygame_menu.Menu('', SCREEN_WIDTH, SCREEN_HEIGHT, theme=theme)
        menu.add.label("ChessAI", align=pygame_menu.locals.ALIGN_CENTER, 
        ...
        menu.add.label("", align=pygame_menu.locals.ALIGN_CENTER, 
                      font_color=BLACK, font_size=70, margin=(0, 50))
        menu.center_content()
    ...
    def determine_move(self):
        ...
        if self.p2_name == "Minimax":
            move = AI.minimax(self.board.copy(), 3, inf, -inf, True, self.p2_color)
            if move is None:
                # Fallback to random move if minimax fails to find a move
                move = AI.random_move(self.board)
            else:
                move = move[0]  # Extract the move from the minimax result tuple
        else:
            move = AI.random_move(self.board)

        # Put the move in the queue if it's valid
        if move is not None:
            self.ai_move.put(move)
